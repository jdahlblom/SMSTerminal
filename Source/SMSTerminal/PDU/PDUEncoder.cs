                ...
                case SMSEncoding._UCS2:
                    {
                        ...
                        var messageLength = Convert.ToString(messageLengthInt, 16).PadLeft(2, '0');
                        //Logger.Debug("[UCS2] TP-User-Data-Length (msg len) = ->{0}<- decimal = {1} BigEndian byte count (msg only) = {2} bytes",messageLength.ToUpper(), messageLengthInt, messageBytes.Length);
